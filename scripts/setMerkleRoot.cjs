...
// Usage: npx hardhat run scripts/setMerkleRoot.cjs --network <network-name>
//
// Prerequisites: Run generateMerkleTree.cjs first to create airdrop-merkle-data.json
...
async function main() {
  ...
  if (!fs.existsSync(merkleDataPath)) {
    ...
    console.error("   Run: npx hardhat run scripts/generateMerkleTree.cjs");
    console.log("");
    process.exit(1);
  }
